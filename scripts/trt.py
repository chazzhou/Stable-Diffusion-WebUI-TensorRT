...
class TrtUnet(sd_unet.SdUnet):
    def __init__(self, model_name: str, configs: List[dict], lora_path, *args, **kwargs):
    ...
    def forward(self, x, timesteps, context, *args, **kwargs):
        ...
        engine = self.engines[self.loaded_config["filepath"]]
        tmp = torch.empty(self.engine_vram_req, dtype=torch.uint8, device=devices.device)
        engine.context.device_memory = tmp.data_ptr()
        self.cudaStream = torch.cuda.current_stream().cuda_stream
        engine.allocate_buffers(feed_dict)

        out = engine.infer(feed_dict, self.cudaStream)["latent"]

        nvtx.range_pop()
        return out

    def switch_engine(self, feed_dict):
        valid_models, distances = modelmanager.get_valid_models(shared.sd_model.sd_checkpoint_info.model_name, feed_dict)
